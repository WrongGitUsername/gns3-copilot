...
if dotenv_loaded:
    logger.info(
        "ExecuteMultipleDeviceCommands Successfully loaded environment variables from .env file"
    )
else:
    logger.warning(
        "ExecuteMultipleDeviceCommands No .env file found or failed to load. Using existing environment variables."
    )
